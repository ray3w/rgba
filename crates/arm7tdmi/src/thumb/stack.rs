...
use super::{load_word, store_word, write_exec_reg, ExecutionResult};

pub(super) fn is_stack(opcode: u16) -> bool {
    (opcode & 0xff00) == 0xb000
...
pub(super) fn execute<B: BusInterface>(
    ...
    if (opcode & 0xff00) == 0xb000 {
        ...
        execute_multiple(cpu, bus, opcode)
    } else {
        unreachable!()
    }
}
...
fn execute_multiple<B: BusInterface>(
    ...
    cpu.write_reg(rb, base.wrapping_add(count * 4));
    ExecutionResult::sequential(count + 1)
}
