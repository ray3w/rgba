...
pub(super) fn execute(cpu: &mut Arm7tdmi, opcode: u32, fetch_pc: u32) -> ExecutionResult {
    ...
    let operand2 = decode_operand2(cpu, opcode, fetch_pc);
    let rn_value = read_exec_reg(cpu, rn, fetch_pc);
    ...
    let wrote_pc = match op {
        ...
        0x8 => {
            update_logical_flags(cpu, rn_value & operand2.value, operand2.carry_out);
            false
        }
        0x9 => {
            update_logical_flags(cpu, rn_value ^ operand2.value, operand2.carry_out);
        ...
        0xa => {
            cpu.set_cpsr(with_arithmetic_flags(
                cpu.cpsr(),
                add_with_carry(rn_value, !operand2.value, true),
            ));
            false
...
fn decode_operand2(cpu: &Arm7tdmi, opcode: u32, fetch_pc: u32) -> ShiftResult {
    if ((opcode >> 25) & 1) != 0 {
        ...
        let value = read_exec_reg(cpu, rm, fetch_pc);
        ...
        if ((opcode >> 4) & 1) != 0 {
            ...
            let amount = (read_exec_reg(cpu, rs, fetch_pc) & 0xff) as u8;
